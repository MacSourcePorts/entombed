...
#define SPRITESIZE_27 27
#define SPRITESIZE_15 15
#define SPRITESIZE_4 4
#define SPRITESIZE_2 2
#define SPRITESIZE_160 160
#define MAKEBREAK_DIST 35
...
#define CREDITS_X 300
#define CREDITS_Y 250

#define CREDITS_W 250
#define CREDITS_H 75

#define PLAYERS_X 304
...
#define SMOKING_SMOKE 20
  ...
  do
	...
	show_num_countdown--;
      ...
      SDL_BlitSurface(many_players_orig, NULL,
		      screen, &dest);
      ...
      while (SDL_PollEvent(&event) > 0)
	{
	  if (event.type == SDL_KEYDOWN)
	      ...
	      else if (key == SDLK_SPACE)
		{
	      ...
	      else if (key == SDLK_KP4 || key == SDLK_KP6 ||
		       key == SDLK_KP8 || key == SDLK_KP2)
		{
		  dirkey[1] = key;
		  last_dirkey[1] = key;
		}
...
int pause(void)
{
  ...
  do
  {
	...
	if (key == SDLK_p || key == SDLK_TAB || key == SDLK_ESCAPE)
	  smoking = 20;
      }
    }
